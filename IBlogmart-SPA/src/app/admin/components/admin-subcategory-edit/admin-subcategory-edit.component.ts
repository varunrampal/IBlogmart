  ...
  constructor(private categoryService: CategoryService,
    ...
    ngOnInit() {
      ...
      this.activatedRoute.params.subscribe((parms) => {
        ...
        this.subCategoryService.getcategory(this.subCatId).subscribe(data => {
    ...
    isNameExists(isExists: boolean) {
      ...
      if (isExists) {
        this.alertifyService.error('Subcategory with the same name already exists');
      }
    }
  
    isEmpty(empty: boolean) {
       this.empty = empty;
       if (empty) { this.alertifyService.error('Please enter subcategory name'); }
